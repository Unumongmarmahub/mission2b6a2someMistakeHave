...
exports.bookingsService = {
    create: async (payload, requester) => {
        ...
        if (!cid || !vehicle_id || !rent_start_date || !rent_end_date)
        ...
        const vr = await db_1.db.query(vq, [vehicle_id]);
        if (vr.rows.length === 0)
        ...
        if (numDays <= 0)
            throw new Error("rent_end_date must be after start date");
      ...
      VALUES ($1,$2,$3,$4,$5,'active') RETURNING id,customer_id,vehicle_id,rent_start_date,rent_end_date,total_price,status`;
        const ir = await db_1.db.query(iq, [cid, vehicle_id, rent_start_date, rent_end_date, total]);
        // update vehicle status
        await db_1.db.query(`UPDATE vehicles SET availability_status='booked' WHERE id = $1`, [vehicle_id]);
        const created = ir.rows[0];
        created.vehicle = { vehicle_name: vehicle.vehicle_name, daily_rent_price: vehicle.daily_rent_price };
        return created;
    },
    getAll: async (requester) => {
        ...
        WHERE b.customer_id = $1 ORDER BY b.id DESC`;
            const r = await db_1.db.query(q, [requester.id]);
            return r.rows;
        }
    },
    updateStatus: async (id, payload, requester) => {
        ...
        if (status === "cancelled") {
            ...
            if (requester.role === "customer" && requester.id !== booking.customer_id)
                throw new Error("Forbidden");
            const today = new Date().toISOString().slice(0, 10);
            if (today >= booking.rent_start_date)
                throw new Error("Cannot cancel on or after start date");
            await db_1.db.query(`UPDATE bookings SET status='cancelled' WHERE id = $1`, [id]);
            await db_1.db.query(`UPDATE vehicles SET availability_status='available' WHERE id = $1`, [booking.vehicle_id]);
        ...
        if (status === "returned") {
            ...
            if (requester.role !== "admin")
                throw new Error("Forbidden");
            await db_1.db.query(`UPDATE bookings SET status='returned' WHERE id = $1`, [id]);
            await db_1.db.query(`UPDATE vehicles SET availability_status='available' WHERE id = $1`, [booking.vehicle_id]);
